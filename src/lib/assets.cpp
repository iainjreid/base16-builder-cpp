...
namespace Assets {
    ...
    void Clone(std::string username, std::string repository) {
     ...
     */
    fs::path Retrieve (fs::path base_path, fs::path repository_path) {
        fs::path asset_path = base_path / repository_path;

        if (!fs::is_directory(asset_path)) {
            Clone(repository_path.string(), asset_path.string());
        }
